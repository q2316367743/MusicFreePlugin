...
const platform = "龙珠API-5sign";
const baseURL = 'https://www.hhlqilongzhu.cn/api/dg_5signmusic.php';
const key = 'msg';
const fileName = 'long-zhu-5sing';
...
async function search<T extends SupportMediaType>(
  ...
  if (type === "music") {
    ...
    const data = rsp.data as Array<any>;
    return {
      isEnd: true,
      // @ts-ignore
      data: data.map((e, i) => {
